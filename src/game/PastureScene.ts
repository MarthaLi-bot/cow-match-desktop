...
function normalizeSvgTextureUrl(url: string) {
  if (url.startsWith('data:image/svg+xml,') && !url.startsWith('data:image/svg+xml;base64,')) {
    return url.replace('data:image/svg+xml,', 'data:image/svg+xml;charset=utf-8,');
  }
...
export class PastureScene extends Phaser.Scene {
  ...
  preload() {
    this.load.on(Phaser.Loader.Events.FILE_LOAD_ERROR, (file: { key: string }) => {
      console.warn(`牧场图标加载失败，使用备用图块: ${file.key}`);
    });

    Object.entries(iconUrls).forEach(([key, url]) => {
      this.load.image(`icon-${key}`, normalizeSvgTextureUrl(url));
    });
  }

  create() {
    this.ensureFallbackIconTexture();
    ...
    [...this.level.tiles]
      ...
      .forEach((tile) => {
        try {
          ...
          console.warn(`图块创建失败，已跳过: ${tile.id}`, error);
        }
      });
  ...
  private ensureFallbackIconTexture() {
    if (this.textures.exists(FALLBACK_ICON_KEY)) return;

    const graphics = this.make.graphics({ x: 0, y: 0 }, false);
    graphics.fillStyle(0xeaf8ff, 1);
    graphics.fillCircle(30, 30, 28);
    graphics.lineStyle(4, 0x8acb66, 1);
    graphics.strokeCircle(30, 30, 26);
    graphics.fillStyle(0x557248, 1);
    graphics.fillCircle(21, 27, 4);
    graphics.fillCircle(39, 27, 4);
    graphics.lineStyle(3, 0x557248, 1);
    graphics.beginPath();
    graphics.arc(30, 36, 10, 0, Math.PI, false);
    graphics.strokePath();
    graphics.generateTexture(FALLBACK_ICON_KEY, 60, 60);
    graphics.destroy();
  }
  ...
  private createTile(tile: TileDef) {
    ...
    const icon = this.createIconImage(tile.icon);
  ...
  private createIconImage(icon: IconId) {
    const textureKey = `icon-${icon}`;
    const safeTextureKey = this.textures.exists(textureKey) ? textureKey : FALLBACK_ICON_KEY;
    return this.add.image(0, -3, safeTextureKey).setDisplaySize(62, 62);
  }
